...
- **nixpkgs-lib:** caisson depends on `nixpkgs-lib` (not full nixpkgs). It consumes the library functions and re-exports them as part of its composed `lib` (via `mkLib`), so downstream projects get nixpkgs-lib functions plus caisson's own extensions through a single dependency.
