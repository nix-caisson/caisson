...
Each integration resolves its ecosystem from the composing flake, in
this order: the `ecosystemSrc` argument, then `defaultEcosystemSrc.<name>`
on `mkLib`, then an input named exactly `<name>`. caisson pins none of
